/**
 * Copy from here - https://github.com/earendil-works/pi-review/blob/main/review.ts
...
8. Use chinese for the output, but keep code snippets and callouts in English as specified.
...
export default function reviewExtension(pi: ExtensionAPI) {
  ...
  async function executeReview(
    ...
    if (useFreshSession) {
      ...
      pi.appendEntry(REVIEW_STATE_TYPE, { active: true, originId: lockedOriginId, target });
    }
...
9. End with: fixed items, deferred/skipped items (with reasons), and verification results.`;
  ...
  /**
   * Extract review result text from assistant messages in the current branch.
   */
  function getReviewResult(ctx: ExtensionContext): string {
    ...
    for (const e of entries) {
      if (e.type === "message" && e.message.role === "assistant") {
        ...
        if (typeof content === "string") {
          ...
          texts.push(content.map((c) => (c.type === "text" ? c.text : "")).join(""));
        }
      }
    }
    return texts.join("\n\n---\n\n");
  ...
  /**
   * Get the latest assistant message text in the current branch.
   */
  function getLatestAssistantMessage(ctx: ExtensionContext): string | null {
    const entries = ctx.sessionManager.getBranch();
  ...
  function formatDiff(diff: string): string {
    if (!diff.trim()) {
      return '<div class="diff-line" style="color:#888;">No diff available for this review target.</div>';
    }
    return diff
      .split("\n")
      .map((line) => {
...
<style>
  * { box-sizing: border-box; }
  ...
  .diff-line { white-space: pre; padding: 1px 6px; border-radius: 3px; }
  .diff-add  { color: #7ee787; background: rgba(46, 160, 67, 0.12); }
  .diff-del  { color: #ffa198; background: rgba(248, 81, 73, 0.12); }
  .diff-hunk { color: #79c0ff; }
  .diff-file { color: #d2a8ff; font-weight: 600; }
...
<div class="container">
  <div class="header">
    ...
    <div class="meta">
      <span>Target: ${escapeHtml(title)}</span>
  ...
  <div class="panel">
    ...
    <div class="panel-body">
      <div class="review-content">${escapeHtml(reviewResult)}</div>
    </div>
  </div>

  <div class="panel">
    <div class="panel-header">Code Diff</div>
    <div class="panel-body">
      <div class="diff-content">${formatDiff(diff)}</div>
    </div>
  </div>
</div>
</body>
  ...
  async function getReviewDiff(pi: ExtensionAPI, target: ReviewTarget): Promise<string> {
    switch (target.type) {
      case "uncommitted": {
        const { stdout } = await pi.exec("git", ["diff", "HEAD"]);
        return stdout;
      }
      case "baseBranch": {
        const mergeBase = await getMergeBase(pi, target.branch);
        const range = mergeBase ? `${mergeBase}..HEAD` : `${target.branch}...HEAD`;
        const { stdout } = await pi.exec("git", ["diff", range]);
        return stdout;
      }
      case "commit": {
        const { stdout: showStdout } = await pi.exec("git", ["show", "--stat", "--format=", target.sha]);
        const { stdout: diffStdout } = await pi.exec("git", ["diff", `${target.sha}~1..${target.sha}`]);
        return `Commit: ${target.sha}${target.title ? " " + target.title : ""}\n${showStdout}\n${diffStdout}`;
      }
      case "pullRequest": {
        const mergeBase = await getMergeBase(pi, target.baseBranch);
        const range = mergeBase ? `${mergeBase}..HEAD` : `${target.baseBranch}...HEAD`;
        const { stdout } = await pi.exec("git", ["diff", range]);
        return stdout;
      }
      case "folder": {
        // Snapshot review: no diff. Optionally show git status for the paths.
        const { stdout } = await pi.exec("git", ["status", "--short", "--", ...target.paths]);
        return stdout
          ? `Git status for reviewed paths:\n${stdout}`
          : "Snapshot review — no diff generated for folder review.";
      }
    }
  }

  async function openHtmlReport(pi: ExtensionAPI, filePath: string): Promise<void> {
    ...
    try {
      ...
      // Silently ignore open failures; the file path is already notified.
    }
  }

  async function generateAndOpenHtmlReport(
    ctx: ExtensionCommandContext,
    target: ReviewTarget,
    reviewContent: string,
    action: EndReviewAction,
  ): Promise<void> {
    ctx.ui.notify("Generating HTML report...", "info");
    const diff = await getReviewDiff(pi, target);
    const html = generateReviewHtml(getUserFacingHint(target), reviewContent, diff, action);
    const fileName = `review-report-${Date.now()}.html`;
    const filePath = path.join(ctx.cwd, fileName);
    await fs.writeFile(filePath, html, "utf8");
    ctx.ui.notify(`HTML report saved: ${filePath}`, "success");
    await openHtmlReport(pi, filePath);
  }
  ...
  async function runEndReview(ctx: ExtensionCommandContext): Promise<void> {
    ...
    try {
      // Capture review result and target before navigation
      const state = getReviewState(ctx);
      const reviewTarget = state?.target;
      const rawReviewResult = getReviewResult(ctx);
      ...
      const result = await executeEndReviewAction(ctx, action, {
        showSummaryLoader: true,
        notifySuccess: true,
      });

      // Ask to generate HTML report after successful review completion
      if (result === "ok" && reviewTarget) {
        const shouldGenerate = await ctx.ui.confirm(
          "Generate HTML Report",
          "Would you like to generate an HTML report with review findings and code diff?",
        );
        if (shouldGenerate) {
          let reviewContent = rawReviewResult;
          if (action === "returnAndSummarize" || action === "returnAndFix") {
            const summary = getLatestAssistantMessage(ctx);
            if (summary) {
              reviewContent = summary;
            }
          }
          await generateAndOpenHtmlReport(ctx, reviewTarget, reviewContent, action);
        }
      }
    } finally {
