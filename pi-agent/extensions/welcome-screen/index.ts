...
const MAX_STACKED_COLUMN_WIDTH = 120;
...
function renderResourceColumn(
  ...
  appendSection(lines, "Skills", resources.skills, theme, columnWidth);
  appendSection(lines, "Prompts", resources.prompts, theme, columnWidth);
  appendSection(lines, "Extensions", resources.extensions, theme, columnWidth);
  return lines;
}
...
export function renderCenteredWelcome(
  ...
  if (width <= 0) return [];
  // Always vertical: logo on top, resources below. Left-aligned for a tight
  // layout — no center indent, content hugs the left edge.
  const layoutWidth = Math.min(MAX_STACKED_COLUMN_WIDTH, width);
  const lines = renderStackedWelcome(resources, theme, layoutWidth);

  return lines.map((line) =>
    line ? truncateToWidth(line, layoutWidth, "") : "",
  );
}
